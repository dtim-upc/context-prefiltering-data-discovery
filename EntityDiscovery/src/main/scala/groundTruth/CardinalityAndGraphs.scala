...
import edu.upc.essi.dtim.nextiadi.config.{DataSourceVocabulary, Namespaces}
...
object CardinalityAndGraphs extends SparkWrapper{
  ...
  case class GraphAndMapProfiles( graph: Graph, mapProfiles: ParMap[String, String] ,fileName: String)

  case class GraphAndProfilesNew( graph: Graph, strProfile: DataFrame, numProfile: Option[DataFrame], boolProfile: Option[DataFrame],fileName: String)
...
//    sourceIDAndGraph
  ...
  def main(args: Array[String]): Unit = {
    ...
    val rawStrProfilesPath =  s"${basePath}/profiles_str_new"
    val rawNumProfilesPath =  s"${basePath}/profiles_num_new"
...
//      .filter(col("active") === true)
    ...
    val gAndStrProfiles = loadSchemaGraphsAndProfiles(parIdAndFileNames, graphPath, rawStrProfilesPath, rawNumProfilesPath)
    setEmptyProperties(gAndStrProfiles, newGraphsPath)
