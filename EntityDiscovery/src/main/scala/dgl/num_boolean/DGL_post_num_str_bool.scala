...
object DGL_post_num_str_bool extends SparkWrapper{
  ...
  def loadNodes(entityNodesPath: String, strNodesPath: String, numNodesPath: String, boolNodesPath: String): (Seq[NodeClass], Seq[NodeStrPost], Seq[NodeNumPost], Seq[NodeBoolPost] )= {
    ...
    val boolDf = spark.read.option("header", "true").option("inferSchema", "true").csv(boolNodesPath)

    validateIDs(entityDf, "entity")
    validateIDs(strDf, "str")
    validateIDs(numDf, "num")
    validateIDs(boolDf, "bool")
    ...
    val numNodes = numDf.collect().map(row =>
    ...
    val boolNodes = boolDf.collect().map(row =>
    ...
    println(s"number bool nodes: ${boolNodes.size} ")

    (entityNodes, strNodes, numNodes, boolNodes)
  ...
  def loadLinks(strLinksPath: String, numLinksPath: String, boolLinksPath:String, relationshipsPath:String, aligPath: String): (Seq[Link], Seq[Link], Seq[Link], Seq[Link], Seq[Link]) = {
    ...
    val boolLinksDf = spark.read.option("header", "true").option("inferSchema", "true").csv(boolLinksPath)
    ...
    validateIDs(boolLinksDf, "boolLinksDf_src", "src_id")
    validateIDs(boolLinksDf, "boolLinksDf_dst", "dst_id")
    ...
    val boolLinks = createLinksFromDF( boolLinksDf)
    ...
    println(s"number bool links ${boolLinks.size}")
    ...
    (strLinks, numLinks, boolLinks, relLinks, aligLinks)
  ...
  def main(args: Array[String]): Unit = {
    ...
    val boolLinksPath = s"${prePath}/bool_links.csv"
    ...
    val boolNodesPath = s"${prePath}/boolNodes.csv"
    val entityNodesPath = s"${prePath}/entityNodes.csv"
    val outputFolder = s"${basePath}/input_dgl_num_str_bool"

    createDirIfNotExists(outputFolder)

    val (entityNodes, strNodes, numNodes, boolNodes) = loadNodes(entityNodesPath, attNodesPath, numNodesPath, boolNodesPath)

    val (strLinks, numLinks,boolLinks, relLinks, aligLinks) = loadLinks(strLinksPath, numLinksPath, boolLinksPath,  relLinksPath, alignmentsLinksPath)
...
//      .filter(col("testing2") === false)
    ...
    writeNodeFilesPost(entityNodes, strNodes, numNodes, boolNodes, outputFolder)
    ...
    writeLinksFile(boolLinks, outputFolder, "g_bool_links.csv")
  ...
  def validateFiles(): Unit = {
    ...
    val outputFolder = "/Users/javierflores/Koofr/PhD/code/result/data/input_dgl_num_str"
