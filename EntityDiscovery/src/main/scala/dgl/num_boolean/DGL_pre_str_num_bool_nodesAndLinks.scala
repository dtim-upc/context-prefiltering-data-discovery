...
import groundTruth.CardinalityAndGraphs.GraphAndProfilesNew
...
import io.github.haross.nuup.nextiajd.profiles.xboolean.{ContainsFalse, ContainsTrue}
...
object DGL_pre_str_num_bool_nodesAndLinks extends SparkWrapper{

  import spark.implicits._


  def loadSchemaGraphsAndProfiles(parIdAndFileNames: ParSeq[(Int, String)], graphsPath: String, profilesStrPath: String, profilesNumPath: String, profilesBoolPath: String): ParSeq[(Int, GraphAndProfilesNew)] = {
    ...
    val sourceIDAndGraph = parIdAndFileNames.flatMap { case (id, fileNameNoExt) =>
      ...
      val numPath = Paths.get(s"${profilesNumPath}/${fileNameNoExt}")
      val boolPath = Paths.get(s"${profilesBoolPath}/${fileNameNoExt}")

      val numProfileDF = if (Files.exists(numPath)) Some(spark.read.parquet(s"${profilesNumPath}/${fileNameNoExt}")) else None
      val boolProfileDF = if (Files.exists(boolPath)) Some(spark.read.parquet(s"${profilesBoolPath}/${fileNameNoExt}")) else None

      Some(id -> GraphAndProfilesNew(graph, strProfileDF, numProfileDF, boolProfileDF, fileNameNoExt))

//      if (Files.exists(numPath)) {
//
//        val numProfileDF = spark.read.parquet(s"${profilesNumPath}/${fileNameNoExt}")
//        Some(id -> GraphAndProfilesNew(graph, strProfileDF, Some(numProfileDF), fileNameNoExt))
//      } else {
//        Some(id -> GraphAndProfilesNew(graph, strProfileDF, None, fileNameNoExt))
//      }
  ...
  def getNodesBool(gAndP: GraphAndProfilesNew, idCounter: AtomicInteger, datasourceId: Int): Seq[NodeBool] = {
    var nodes = Seq.empty[NodeBool]

    val query = s"SELECT ?node ?label ?alias ?domain WHERE { " +
      s" ?node <${RDF.`type`}> <${RDF.Property}> . " +
      s" ?node <${RDFS.label}> ?label . " +
      s" ?node <${RDFS.range}> <${XSD.xboolean}> . " +
    ...
    while (result_nodes.hasNext) {
      ...
      nodes = nodes :+ NodeBool(
        ...
        profile = getNodeBoolProfile(alias, gAndP.boolProfile.get, gAndP.fileName),
        alias = Some(alias),
  ...
  def getNodesNum(gAndP: GraphAndProfilesNew, idCounter: AtomicInteger, datasourceId: Int): Seq[NodeNum] = {
  ...
  def getNodesStr(gAndP: GraphAndProfilesNew, idCounter: AtomicInteger, datasourceId: Int): Seq[NodeStr] = {
  ...
  def getNodeBoolProfile(alias: String, boolProfileDF: DataFrame, fileName: String): Option[ProfilesBoolRow] = {

    val df_tmp = boolProfileDF.filter(col(Commons.attColumn) === alias).select(Seq(ContainsTrue.nameAtt, ContainsFalse.nameAtt).map(col).map(_.cast(StringType)): _*)
    if (!df_tmp.head(1).isEmpty) {

      val row = df_tmp.first()

      val p = ProfilesBoolRow(
        row.getAs[Any](ContainsTrue.nameAtt).toString, row.getAs[Any](ContainsFalse.nameAtt).toString,
      )

      Some(p)
    } else {
      println(s"----> cannot find bool profile for field ${alias} in dataset ${fileName}. ")
      None
    }

  }

  def getNodesEntity(gAndP: GraphAndProfilesNew, idCounter: AtomicInteger, datasourceId: Int): Seq[NodeClass] = {
  ...
  def loadNodes(graphAndProfile: ParSeq[(Int, GraphAndProfilesNew)]): (Seq[NodeClass], Seq[NodeStr],  Seq[NodeNum], Seq[NodeBool], Map[String, Int]) = {
    ...
    val boolAttCounter = new AtomicInteger(0)
    ...
    println("loading boolean nodes")
    val parBoolAttNodesNoDomain: ParSeq[NodeBool] = graphAndProfile.flatMap { case (id, gAndP) =>
      getNodesBool(gAndP, boolAttCounter, id)
    }
    ...
    val parBoolAttNodes= setDomainBoolIDs(entityIriAndIDs, parBoolAttNodesNoDomain).seq

    (parEntityNodes.seq, parStrAttNodes, parNumAttNodes, parBoolAttNodes, entityIriAndIDs)
  }
  ...
  def setDomainBoolIDs(iriAndIDs: Map[String, Int], boolNodes: ParSeq[NodeBool]): ParSeq[NodeBool] = {

    // it's thread-safe since we are modifying each element but their data does not depend on other elements from the same seq
    val attNodes = boolNodes.map { n =>

      if (n.domainURI.isEmpty) {
        println(s"ERROR: bool attribute ${n.name} with iri ${n.iri} does not have a domain. Problem might be when loading nodes")
        n
      } else {
        n.copy(domainID = Some(getIDNode(n.domainURI.get, iriAndIDs)))
      }
  ...
  def loadLinks(graphAndProfile: ParSeq[(Int, GraphAndProfilesNew)], entityIriAndIDs: Map[String, Int], strAttNodes: Seq[NodeStr], numNodes: Seq[NodeNum], boolNodes: Seq[NodeBool]): (ParSeq[Link], ParSeq[Link],ParSeq[Link],ParSeq[Link], Map[String, Int]) = {
    ...
    val boolIriAndIDs = boolNodes.map(node => (node.iri -> node.id)).seq.toMap
    val parBoolLinks = graphAndProfile.flatMap { case (id, gAndP) =>
      getLinksBoolAtt(gAndP.graph, entityIriAndIDs, boolIriAndIDs)
    }



    (parRelationshipLinks, parStrLinks, parNumLinks,parBoolLinks, strIriAndIDs)
  ...
  def getLinksBoolAtt(graph: Graph, entityIriAndIDs: Map[String, Int], boolIriAndIDs: Map[String, Int]): Seq[Link] = {

    var links = Seq.empty[Link]
    val query = s"SELECT ?source ?property WHERE { " +
      s" ?property <${RDFS.domain}> ?source. " +
      s" ?property <${RDFS.range}> <${XSD.xboolean}> . " +
      s" FILTER NOT EXISTS { ?property <${VocabularyCore.metadata}> <${VocabularyCore.emptyAtt}> . } " +
      s" } "
    ...
    while (result_nodes.hasNext) {
      ...
      val boolID = getIDNode(target, boolIriAndIDs, Commons.defaultBoolDomainID, "bool attribute")
      links = links :+ Link(getIDNode(source, entityIriAndIDs), boolID, Commons.linkTypes("boolAttribute"), "boolAttribute")
    }
    links
  }
  ...
  def createAlignmentsByRepositories(dsInfo: DataFrame, graphAndProfile: ParSeq[(Int, GraphAndProfilesNew)], entityIriAndIDs: Map[String, Int]): Seq[Link] = {
...
//          println(s"alignment for ${idAndFileA.id} - ${idAndFileB.id}")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val boolProfilePathByDS = s"${basePath}/profiles_bool_new"
...
//      .filter(col("testing2") === false)
    ...
    val gAndProfiles = loadSchemaGraphsAndProfiles(parIdAndFileNames, graphsWithMetadataPath, profilePathNormByDS, numProfilePathNormByDS, boolProfilePathByDS)
    println("Loading nodes")
    val (entityNodes, strAttNodes, numNodes, boolNodes, entityIriAndIDs) = loadNodes(gAndProfiles)

    println("entity nodes" ,entityNodes.size)
    println("str nodes", strAttNodes.size)
    println("num nodes", numNodes.size)
    println("bool nodes", boolNodes.size)

    println("Loading links")
    val (relationshipLinks, attrLinks, numLinks, boolLinks, strIriAndIDs) = loadLinks(gAndProfiles, entityIriAndIDs, strAttNodes, numNodes, boolNodes)
    ...
    writeNodeFiles(entityNodes, strAttNodes, numNodes , boolNodes, outputFolder)
    ...
    writeLinksFile(boolLinks.seq, outputFolder, "bool_links.csv")
