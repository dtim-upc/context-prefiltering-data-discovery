...
def main_lp(args):
    ...
    for i in range(args.repeat):
        ...
        if args.model == 'MECCH':
            if args.ablation:
            ...
            else:#g_train.to_canonical_etype(k) returns the tuple ENTITY- ENTITY-alignment-ENTITY for key ENTITY-alignment-ENTITY
                ...
                n_heads_list = [args.n_heads] * args.n_layers
                # It seems the MECCH class represents the core architecture of the Metapath GNN model
                # while the LinkPrediction_minibatch class is a wrapper for the link prediction task that leverages the embeddings produced by the MECCH model.
                model = MECCH(
                    g_train,
                    selected_metapaths,
                    in_dim_dict,
        ...
        if minibatch_flag:
            print("minibath...flag...")
            test_auroc, test_ap, trained_model = link_prediction_minibatch(model_lp, g_train, g_val, g_test, train_eid_dict,
                                                            val_eid_dict, test_eid_dict, val_neg_uv, test_neg_uv,
                                                            dir_path_list[i], args)

            load_path_model = Path('./data') / args.dataset / 'nextia_model.pth'
            th.save(trained_model.state_dict(), load_path_model)
        else:
